...
(async () => {
  try {
    const emailAddress = Office.context?.userProfile?.email || "not_found";
    await logExecution("🔍 Resolving email...");

    if (emailAddress === "not_found") {
      const stopMsg = "⛔ No email address found. No license found - AI usage is not allowed.";
      console.log(stopMsg);
      await logExecution(stopMsg);
      return;
    }

    await logExecution("✅ Email resolved: " + emailAddress);

    const licenseUrl = 'https://script.google.com/macros/s/AKfycbxVROg7JFfR_nsq_Wxdg-ftBJwCIpEZqI1G6oWMrOkENgAMGlljFx9tjU0ToiC5ugcnHA/exec?userid=' + encodeURIComponent(emailAddress);
    console.log("📡 Calling license URL:", licenseUrl);

    const response = await fetch(licenseUrl);
    const result = await response.text();

    console.log("📥 License check result:", result);
    await logExecution("📥 License check result: " + result);

    await Excel.run(async (context) => {
      if (result.includes("License is valid")) {
        isLicensedUser = true;
        const msg = `✅ License valid for: ${emailAddress}`;
        context.workbook.application.statusBar = msg;
        await logExecution(msg);
      } else {
        const msg = `⚠️ No license for: ${emailAddress}. AI usage is not allowed.`;
        context.workbook.application.statusBar = msg;
        await logExecution(msg);
      }
      await context.sync();
    ...
    await Excel.run(async (context) => {
      const errMsg = `❌ License check failed. Error: ${error.message}`;
      context.workbook.application.statusBar = errMsg;
      await context.sync();
      await logExecution(errMsg);
    });
  }
})();
  ...
  if (budgetBtn) {
    budgetBtn.onclick = async () => {
      await Excel.run(async (context) => {
        ...
        // Delete if existing
        const sheetNames = ["docs.FIS.Data", "docs.FIS.ACCOUNTS", "menu.FIS.D4ESQL", "FISSQL", "menu", "menuflow","Macro2"];
...
async function logExecution(message) {
  await Excel.run(async (context) => {
    const sheets = context.workbook.worksheets;
    const sheetName = "menulog";
    ...
    if (sheet.isNullObject) {
      sheet = sheets.add(sheetName);
      await context.sync(); // Must sync before modifying
    }

    // Always check and fix header in A1
    const headerRange = sheet.getRange("A1");
    headerRange.load("values");
    await context.sync();

    const currentHeader = (headerRange.values && headerRange.values[0] && headerRange.values[0][0]) || "";
    if (currentHeader !== "LogMessage") {
      headerRange.values = [["LogMessage"]];
    ...
    sheet.getRange(`A${nextRow}`).values = [[logText]];
...
async function testEmailIdResolution() {
  try {
    const emailContext1 = Office.context?.userProfile?.email || "not_found";
    const emailContext2 = Office.context?.mailbox?.userProfile?.emailAddress || "not_found";

    console.log("Email (Office.context.userProfile.email):", emailContext1);
    console.log("Email (Office.context.mailbox.userProfile.emailAddress):", emailContext2);

    await logExecution("TEST: Office.context.userProfile.email = " + emailContext1);
    await logExecution("TEST: Office.context.mailbox.userProfile.emailAddress = " + emailContext2);

  } catch (error) {
    console.error("Email resolution test failed:", error);
    await logExecution("❌ Email resolution test failed: " + error.message);
  }
}

document.addEventListener("keydown", function (event) {
  if (event.key === "F3") {
    testEmailIdResolution();
  }
});
